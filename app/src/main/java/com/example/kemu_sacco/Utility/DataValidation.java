...
// full name  - must letters -- no- number, no special symbol
    ...
    public static Boolean isNotValidID(String ID_NUMBER){
        Boolean valid =true;
        if (!TextUtils.isEmpty(ID_NUMBER.trim())){
            if (ID_NUMBER.trim().length() == 8){
                valid = false;
            }
        }
        return valid;
    ...
    public static Boolean isValidPhoneNumber(String phonenumber){
        Boolean valid =true;
        if (!TextUtils.isEmpty(phonenumber.trim()) && phonenumber.length() == 10){
